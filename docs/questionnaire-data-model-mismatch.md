...
It also mixes entities that the data model deliberately separates. “Object(s) shown” may refer to one physical object with several photographed pages, several objects already combined in one image, or several image files that should be displayed as a single side-by-side composition. Gallery image groups can represent the last case as one slide containing several image references. Captions and credits may apply to one image, a whole group, or the underlying objects. Spatial labels such as “links”, “rechts”, and “vorne” describe the relationship between an object and a particular composite; they belong on that object's image association, not on the image metadata or as permanent properties of the object.
