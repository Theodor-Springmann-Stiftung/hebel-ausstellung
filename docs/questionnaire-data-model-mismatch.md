...
Store reusable facts on objects and view-specific facts on images. In particular, composite positions belong in `images.objektPositionen`, in the same order as `images.objekte`; storing them on an object makes “Links” or “Rechts” leak into unrelated views. If several files must appear as one visual unit, do not silently flatten them into independent gallery slides. Use a prepared montage, extend the model with an explicit image-group concept, or record the layout request as unresolved until the presentation behavior is decided.
